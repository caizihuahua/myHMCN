...
# plt.cla();plt.clf();nx.draw(g);plt.savefig('gg')
# Skip the root nodes 
to_skip = ['root', 'GO0003674', 'GO0005575', 'GO0008150']

# self.A=adjacency matrix, self.terms=all y-label self.g=DiGraph
class arff_data():
    def __init__(self, arff_file, is_GO, is_test=False):
        self.X, self.Y,self.Yl,self.A, self.terms, self.g = parse_arff(arff_file=arff_file, is_GO=is_GO, is_test=is_test)        
...
# split global label into local one
def split_global_label(yg,nodes):
    yl = {}
    n = np.array([s.count('.') for s in nodes]).astype(float)
    # for dataset: funcat_cellcycle, there are total 5 hierarchical layers
    ntype = int(n[-1])
    ylidx = np.zeros(ntype+1).astype(int)
    for i in range(1,ntype+1,1):
        ylidx[i] = int(sum(n<=i))
        yl[i] = yg[:,ylidx[i-1]:ylidx[i]]
    # yl[i], count number of "." is i
    return (yl[1],yl[2],yl[3],yl[4],yl[5])
    ...
    with open(arff_file) as f:
        ...
        for num_line, l in enumerate(f):
            if l.startswith('@ATTRIBUTE'):
                ...
                lab = d_line[len(feature_types)].strip() #xxx@xx@xxxxxx, each xxx is a hierarchical label
                # what's this mean
                X.append(list(chain(*[feature_types[i](x,i) for i, x in enumerate(d_line[:len(feature_types)])])))
                # ylabel=nodes_(20.01.01)(500), xlabel=arr(77)
                for t in lab.split('@'):   
                    y_[[nodes_idx.get(a) for a in nx.ancestors(g_t, t.replace('/', '.'))]] =1 # set its ancestors
                    y_[nodes_idx[t.replace('/', '.')]] = 1 # set it self
                Y.append(y_)
        X = np.array(X)
        Y = np.stack(Y)
        Yl = split_global_label(Y,nodes)
    #cellcycle_FUN.train X.shape=[1628,77] Y.shape=[1628,500], g the hierarchical-label-digGraph
    return X, Y,Yl,nx.to_numpy_array(g, nodelist=nodes), nodes, g
